...
if(!isMatch){
  ...
  const token = jsonWebToken.sign({id:user._id},process.env.JWT_SECRET,{expiresIn:"7d"});// json web token is used to send the user id to the client side in the form of token so that the client can use it to access the protected routes. it generates a token using the user id and the secret key which is stored in the environment variable. the token is then sent to the client side in the form of cookie so that the client can use it to access the protected routes.
  console.log(token);
